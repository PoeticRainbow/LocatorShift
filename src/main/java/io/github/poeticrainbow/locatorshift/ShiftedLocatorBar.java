...
public class ShiftedLocatorBar extends LocatorBar {
    ...
    public static void renderWaypointAsPlayerHead(DrawContext context, TrackedWaypoint waypoint, int y) {
        ...
        if (source.isPresent()) {
            ...
            double relativeYaw = waypoint.getRelativeYaw(client.world, client.gameRenderer.getCamera(), entity -> 0f);
            ...
            var distance = (int) Math.sqrt(waypoint.squaredDistanceTo(client.getCameraEntity()));
    ...
    public static Optional<PlayerListEntry> getEntryForWaypoint(UUID uuid) {
    ...
    private static void renderPlayerHead(DrawContext context, SkinTextures texture, int x, int y, int brightness, int scale) {
        ...
        PlayerSkinDrawer.draw(context, texture.body().texturePath(), x - centerOffset, y - centerOffset, scale, true, false,
